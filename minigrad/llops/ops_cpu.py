...
class CPUBuffer(np.ndarray,GenericExecAST):
    ...
    fxn_for_op = {
        ...
        MovementOps.RESHAPE: lambda x, shape: x.reshape(shape).view(CPUBuffer),
        ...
        ReduceOps.MAX: lambda x, axis,keepdim: x.amax(axis=axis,keepdims=keepdim).view(CPUBuffer),
    ...
    # object methods
    def masked_fill(x,mask,value): x[mask]=value; return x.view(CPUBuffer)
    def relu(x): return np.maximum(0,x)
    def log(x): return np.log(x)
    def exp(x): return np.exp(x)
    def sign(x): return np.sign(x)
    def float(x): return x.astype(np.float32).view(CPUBuffer)
    def flip(x,axis): return np.flip(x,axis).view(CPUBuffer)
    def amax(x,*args,**kwargs): return np.amax(x,*args,**kwargs).view(CPUBuffer)
