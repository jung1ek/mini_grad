...
def helper_test_op(shapes, torch_fn, minigrad_fxn, atol=1e-6, rtol=1e-3,grad_atol=1e-4,grad_rtol=1e-3, vals=None, a=-0.5, b=3, forward_only=False):
    ...
    if shapes is None:
        ...
        ts = [torch.tensor((np.random.random(size=s).astype(np.float32)+a)*b, requires_grad=True) for s in shapes]
    # minigrad Tensors
    ...
    compare_("forward_pass", ret.numpy(),out.detach().numpy(),atol=atol,rtol=rtol)

    torch_fbp, minigrad_fbp = np.nan, np.nan
    if not forward_only and not FORWARD_ONLY:
        ...
        minigrad_fbp = time.monotonic()-st

        for i, (t,mt) in enumerate(zip(ts,tst)):
            compare_(f"Backward pass tensor{i}", mt.grad.numpy(), t.grad.detach().numpy(), atol=grad_atol, rtol=grad_rtol)
    print("\ntesting %40r torch/minigrad fp: %.2f / %.2f ms bp: %.2f / %.2f ms "% (shapes, torch_fp*1000, minigrad_fp*1000, torch_fbp*1000, minigrad_fbp*1000), end="")
...
class TestOps(unittest.TestCase):
    ...
    def test_log(self):
        helper_test_op([(45,65)], lambda x: torch.log(x), Tensor.log,a=1)
    def test_exp(self):
    ...
    def test_broadcast_full(self):
        for torch_op, minigrad_op in [(torch.add, Tensor.add), (torch.sub, Tensor.sub), (torch.mul, Tensor.mul),
                                    (torch.div, Tensor.div), (torch.pow, Tensor.pow)]:
            for shapes in [((5,13,24,16), (5,1,24,1)), ((1,3,1,7,1), (2,1,5,1,8))]:
                with self.subTest(op=torch_op.__name__, shapes=shapes):
                    helper_test_op(shapes, torch_op, minigrad_op, a=-0.5 if minigrad_op != Tensor.pow else 0.0)
    ...
    def test_reshape(self):
        helper_test_op([(4,3,6,6)], lambda x: torch.reshape(x, (-1,3,6,6)), lambda x: x.reshape(*(-1,3,6,6)))
        helper_test_op([(4,3,6,6)], lambda x: torch.reshape(x, (-1,1,6,6)), lambda x: x.reshape(*(-1,1,6,6)))
    ...
    def test_maxpool2d_simple(self):
        ...
        helper_test_op([(1,1,2,3)],
            ...
            lambda x: Tensor.max_pool2d(x, kernel_size=ksz),forward_only=False)
